...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		WHERE tbl_attendance.lecturer_id = '".$_SESSION["lecturer_id"]."' AND (
		";
		...
		else
		{
			...
			else
			{
				for($count = 0; $count < count($student_id); $count++)
				{
					...
					(student_id, attendance_status, attendance_date, lecturer_id)
					VALUES (:student_id, :attendance_status, :attendance_date, :lecturer_id)
					";
					$statement = $connect->prepare($query);
